 ...
 * |  Functuion: 阿里云直播API接口的封装
 * |  Help: https://help.aliyun.com/document_detail/48207.html
 * '------------------------------------------------------------------------------------------------------------------*/
...
class AliLive
{
     ...
     * @return array
     */
    public static function createPushFlowAddress($sourceName, $domainName, $appName, $expireTime, $authKeyStatus = 0,$cdn)
    {
        $streamName = '800' . time();
        ...
        $authenticationUrl = self::getOpenPushUrl($sourceName, $domainName, $appName, $streamName);

        $responseResult = [
            'domainName' => $domainName,
            'appName' => $appName,
            'streamName' => $streamName,
            'push_flow_address' => $authenticationUrl,
            'play_rtmp_address' => "rtmp://$domainName/$appName/$streamName",
            'play_flv_address' => "http://$domainName/$appName/$streamName.flv",
            'play_m3u8_address' => "http://$domainName/$appName/$streamName.m3u8",
            'expireTime' => $expireTime,
            'createTime' => getCurrentDate()
        ];
        return $responseResult;
