 ...
 * |  DateTime: 2018/2/26 16:13
 * |  Mail: Overcome.wan@Gmail.com
 * |  Function: 网易云IM
 * |  Help Doc: http://www.bcty365.com/content-83-5949-1.html
 * '------------------------------------------------------------------------------------------------------------------*/
...
class YunXinIM
{
    ...
    public function postDataCurl($url, $data)
    {
        ...
        foreach ($data as $key => $value) {
            array_push($postDataArr, $key . '=' . urlencode($value));
        }
    ...
    /**
     * 创建云信ID
     * 注意：创建网易云通信ID时如果不指定此token，则网易云通信会为该ID生成一个加密的token，第三方需要保存此token
     * 1.第三方帐号导入到云信平台；
     ...
     * @return array
     */
    public function createAccId($accId, $name = '', $props = '{}', $icon = '', $token = '')
    {
     ...
     * @param  $props [json属性，第三方可选填，最大长度1024字节]
     * @param  $token [云信ID可以指定登录token值，最大长度128字节，并更新，如果未指定，会自动生成token，并在创建成功后返回]
     * @return array    [返回array数组对象]
     */
    public function updateAccId($accId, $name = '', $props = '{}', $token = '')
    {
     ...
     * @return array    [返回array数组对象]
     */
    public function updateAndGetToken($accId)
    {
     ...
     * @param string $name 云信ID昵称
     * @param string $icon 用户icon
     * @param string $sign 用户签名
     * @param string $email 用户email
     * @param string $birth 用户生日
     * @param string $mobile 用户mobile
     ...
     * @return array
     */
    public function setDonnop($accId, $donnopOpen = true)
    {
     ...
     * @param string $ex 修改ex字段，限制长度256
     * @return array
     */
    public function updateFriend($accId, $faccId, $alias = '', $ex = '')
    {
     ...
     * @param  string $relationType 本次操作的关系类型,1:黑名单操作，2:静音列表操作
     * @param  string $value 操作值，0:取消黑名单或静音，1:加入黑名单或静音
     * @return array
     */
