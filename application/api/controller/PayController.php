...
class PayController
{
    ...
    public function returnUrl(Request $request)
    {
        ...
        if (!empty($data)) {
            ...
            if (empty($orderInfo)) {
                Log::error(getCurrentDate() . '订单不存在');
            }
            echo '同步结果 success';
            halt($data);
        }
    ...
    public function notifyUrl(Request $request)
    {
        ...
        if ($request->isPost()) {
            ...
            Log::error(getCurrentDate() . '-支付宝异步通知数据-' . json_encode($postData));
            $alipay = Pay::alipay(config('alipay'));
            try {
...
//                Log::error(getCurrentDate().'订单金额与发起支付金额不一致=》'.json_encode($data));
//            }
            Log::error(getCurrentDate() . "订单状态更新开始");
            ...
            if ($data['trade_status'] == "TRADE_SUCCESS") {
                ...
                Log::error(getCurrentDate() . "订单状态更新结束");
            }
            // 信息通知
    ...
    public static function sendMsg($orderInfo)
    {
        send_email_qq('756684177@qq.com', '支付成功信息通知', $orderInfo['snap_name']);
        //send_dayu_sms(13669361192, "register", ['code' => rand(100000, 999999)]);
    }
}