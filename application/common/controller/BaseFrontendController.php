...
class BaseFrontendController extends BaseController
{
    ...
    public function _initialize()
    {
        ...
        $userInfo = OpenUser::where('id', session('open_user_id'))
        ...
        $tags = Db::table('resty_tag')
            ...
            ->cache('RESTY_TAG')
            ->select();
        $tagCounts = Tag::where('id','>',1)
            ->cache("RESTY_TAG_COUNTS")
            ->count();

        // 友情链接
        $links = Link::where('id','>',1)
            ->order('sort desc')
            ->cache("RESTY_LINK")
            ->select();
        // 分类信息
        $categorys = Category::where('id','>',1)
            ...
            ->select();
        $categoryCounts = Tag::where('id','>',1)
            ->cache('RESTY_CATEGORY_COUNT')
            ->count();

        // 导航栏
        $navbars = Navbar::where('is_display', 1)
            ->order('sort asc')
            ->cache('RESTY_NAVBAR')
            ->select();
